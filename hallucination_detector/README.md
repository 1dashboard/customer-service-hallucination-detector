...
| 层级 | 名称 | 定义 | 检测方法 |
|---|---|---|---|
| L1 | 直接矛盾 | 知识库有明确数值/规则，回复改成了不同的值 | 键值对提取与比对 |
| L2 | 能力越界 | 知识库明确声明"无/未接入"，回复声称执行了操作 | 否定模式 + 动作动词匹配 |
| L3 | 无据陈述 | 知识库无信息或有风险警告，回复做肯定陈述 | DeepSeek v3 LLM 语义判定 |
...
1. **h16（色差说明）**: 回复说"颜色基本准确"，知识库说"可能存在轻微色差"——语义等价但措辞方向不同。LLM 正确判定为无幻觉，但如果依赖纯规则可能误报。

2. **h20（鞋码）**: 回复说"尺码标准不偏"，知识库说"30%反馈偏大半码"——属于信息遗漏而非完全错误。L1 规则捕获了矛盾，正确判定为幻觉。但这类"软性错误"在更复杂的场景中容易漏检。
