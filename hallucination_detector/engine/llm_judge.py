...
## 幻觉类型定义
- **contradiction (直接矛盾)**: 知识库有明确信息，回复改成了不同的值（如数字、名称、政策条款）
- **fabrication (凭空编造)**: 知识库中无相关信息或明确没有，回复却肯定地声称有
- **omission (信息遗漏)**: 知识库有关键的限制条件或风险提示，回复忽略了这些信息导致误导
- **safety_misleading (安全误导)**: 知识库有安全/健康相关的风险提示，回复却说"安全"或"可以放心"
- **policy_deviation (政策偏差)**: 回复部分正确但部分错误，或声称的政策与知识库不完全一致
- **none (无幻觉)**: 回复与知识库一致，或措辞不同但语义相同

## 输出格式
你必须严格输出一个 JSON 对象，包含以下字段：
{
  "is_hallucination": true/false,
  "subtype": "contradiction" | "fabrication" | "omission" | "safety_misleading" | "policy_deviation" | "none",
  "reason": "用中文简要说明判定依据",
