...
Core hallucination detection engine for customer service replies. Detects when LLM-generated replies fabricate policies, parameters, capabilities, or omit critical safety information. Uses a two-layer classification (detection layer for engine decisions + output layer for business reporting) and a dual-stage pipeline (rule engine for deterministic cases, LLM for semantic judgment).
