...
检测层（3 类，面向检测方法）
层级	名称	定义	检测方法
L1	直接矛盾	知识库 有明确数值/规则，回复改成了不同的值	键值对提取与比对
L2	能力越界	知识库 明确声明"无/未接入"，回复声称执行了操作	否定模式 + 动作动词匹配
L3	无据陈述	知识库 无信息或有风险警告，回复做肯定陈述	DeepSeek v3 LLM 语义判定
输出层（8 类，面向业务归因）
输出类型	说明	严重程度
参数编造	产品参数被篡改	高
政策编造	编造不存在的政策	高
...
召回率（Recall）	100.00%
F1 分数（F1 Score）	100.00%
真阳性（True Positives）	18
真阴性（True Negatives）	2
误报（False Positives）	0
...
h16（色差说明）: 回复说"颜色基本准确"，知识库 说"可能存在轻微色差"——语义等价但措辞方向不同。LLM 正确判定为无幻觉，但如果依赖纯规则可能误报。

h20（鞋码）: 回复说"尺码标准不偏"，知识库 说"30%反馈偏大半码"——属于信息遗漏而非完全错误。L1 规则捕获了矛盾，正确判定为幻觉。但这类"软性错误"在更复杂的场景中容易漏检。

h13（孕妇面膜）: 需要理解"视黄醇衍生物对孕妇的风险"这一外部医学知识。L1 规则通过 KV 提取（孕妇可用: 咨询医生→是）成功发现矛盾，避免了纯依赖 LLM 的知识盲区风险。

输出类型细粒度映射: 二分类准确率 100%，但部分 case 的输出类型（如 h05 优惠编造→param_fabrication）存在映射偏差，后续可通过扩展分类规则优化。
